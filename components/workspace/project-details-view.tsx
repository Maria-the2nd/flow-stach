...
function ProjectContent({
  ...
  const generateUploadUrl = useMutation(
    api.projects.generateThumbnailUploadUrl,
  );
  ...
  const tokenWebflowJsonArtifact = artifacts.find(
    (a) => a.type === "token_webflow_json",
  );
  const jsHooksArtifact = artifacts.find((a) => a.type === "js_hooks");
  const externalScriptsArtifact = artifacts.find(
    (a) => a.type === "external_scripts",
  );
  const externalStylesheetsArtifact = artifacts.find(
    (a) => (a.type as string) === "external_stylesheets",
  );
  const securityWarningsArtifact = artifacts.find(
    (a) => (a.type as string) === "security_warnings",
  );
  // Note: class_renaming_report is a newer artifact type that may not be in all projects
  const classRenamingArtifact = artifacts.find(
    (a) => (a.type as string) === "class_renaming_report",
  );
  ...
  if (cssArtifact?.content) {
    ...
    console.log(
      `[project-details] :nth-child in stored CSS: ${hasNthChildStored}`,
    );
    if (hasNthChildStored) {
      const nthChildRulesStored = cssArtifact.content.match(
        /\.[^{]+:nth-child\([^)]+\)\s*\{[^}]+\}/g,
      );
      console.log(
        `[project-details] :nth-child rules in stored CSS:`,
        nthChildRulesStored?.slice(0, 3),
      );
    }
    ...
    console.log(
      `[project-details] :nth-child in embed CSS: ${hasNthChildEmbed}`,
    );
    if (hasNthChildEmbed) {
      const nthChildRulesEmbed = cssRouting.embed.match(
        /\.[^{]+:nth-child\([^)]+\)\{[^}]+\}/g,
      );
      console.log(
        `[project-details] :nth-child rules in embed CSS:`,
        nthChildRulesEmbed?.slice(0, 3),
      );
    }
    ...
    if (cssRouting.embed && cssRouting.embed.trim()) {
      embeds.push({
        ...
        description:
          'CSS variables, media queries, animations, and complex selectors. In Webflow: Press A → Search "Embed" → Place at TOP of page → Paste inside.',
      });
    }
    ...
    if (cssRouting.stats.totalRules > 0) {
      console.log(
        `[CSS Routing] ${cssRouting.stats.nativeRules} native, ${cssRouting.stats.embedRules} embed, ${cssRouting.stats.atRulesExtracted} at-rules`,
      );
    }
  }
  ...
  if (jsArtifact?.content) {
    embeds.push({
      ...
      content: `<script>\n${jsArtifact.content}\n</script>`,
      description:
        'Component scripts and interactions. In Webflow: Page Settings (gear icon) → Custom Code → Paste in "Before </body> tag" section.',
    });
  }
  ...
  if (jsHooksArtifact?.content) {
    try {
      ...
      if (
        Array.isArray(hooks) &&
        hooks.length > 0 &&
        hooks.every((hook) => typeof hook === "string")
      ) {
        embeds.push({
          type: "JavaScript",
          label: "Event Hooks",
          ...
          description:
            "Interactive event handlers. In Webflow: Add after main scripts in Page Settings → Custom Code → Before </body> tag.",
        });
      }
    } catch (e) {
  ...
  const siteStructurePayload = useMemo<WebflowPayload | null>(() => {
    try {
          ...
          const tokenPayload = JSON.parse(tokenWebflowJsonArtifact.content) as {
            payload?: {
              styles?: Array<{
                name?: string;
                _id?: string;
                styleLess?: Record<string, string>;
          ...
          if (Array.isArray(tokenPayload.payload?.styles)) {
            tokenPayload.payload.styles.forEach((style) => {
              ...
              if (styleName && styleId) {
                ...
                allStyles.push(
                  style as unknown as WebflowPayload["payload"]["styles"][number],
                );
      ...
      components.forEach(({ payload }) => {
        if (payload?.webflowJson) {
          try {
            ...
            const componentStyleById = new Map<
              string,
              WebflowPayload["payload"]["styles"][number]
            >();
            if (Array.isArray(regenerated.payload?.styles)) {
              regenerated.payload.styles.forEach((style) => {
                ...
                const existingCanonicalId =
                  canonicalStyleIdByName.get(styleName);
                    ...
                    if (style) {
                      ...
                      if (tokenStyleNames.has(styleName)) {
                        const canonicalTokenId =
                          tokenStyleIdByName.get(styleName);
                        if (canonicalTokenId) {
      ...
      if (unresolvedClassRefs.length > 0) {
        console.warn(
          "Unresolved class references in Site Structure payload:",
          unresolvedClassRefs.slice(0, 10),
        );
      }
  ...
  const handleAttemptFix = async (): Promise<{
    fixed: number;
    remaining: number;
  }> => {
    ...
    try {
      ...
      const beforeReport = ensureWebflowPasteSafety({
        payload: currentPayload,
      });
      const beforeIssues =
        beforeReport.report.warnings.length +
        beforeReport.report.fatalIssues.length;
      ...
      const safetyResult = ensureWebflowPasteSafety({
        payload: siteStructurePayload,
      });

      // The safety gate's returned `payload` includes all auto-fixes
      if (
        safetyResult.sanitizationApplied ||
        safetyResult.report.autoFixes.length > 0
      ) {
        setFixedPayload(safetyResult.payload);

        // Get new issue count after fix
        const afterReport = ensureWebflowPasteSafety({
          payload: safetyResult.payload,
        });
        const afterIssues =
          afterReport.report.warnings.length +
          afterReport.report.fatalIssues.length;

        const fixed = Math.max(0, beforeIssues - afterIssues);
        if (safetyResult.report.autoFixes.length > 0) {
          toast.success(
            `Applied ${safetyResult.report.autoFixes.length} auto-fix(es)`,
          );
        } else {
      ...
      toast.info(
        "No additional fixes available - remaining issues may need manual resolution in Webflow",
      );
      ...
      <div className="flex items-center gap-4 sm:gap-6 mb-10">
        <Link href="/workspace/projects" className="shrink-0">
          <Button
            variant="ghost"
            size="icon"
            className="h-10 w-10 hover:bg-accent text-muted-foreground hover:text-foreground rounded-xl transition-all"
          >
        ...
        <div className="flex">
          <div className="bg-card/90 backdrop-blur-xl rounded-[32px] border border-white/20 ring-1 ring-white/10 p-8 shadow-2xl shadow-primary/5 h-full flex flex-col w-full">
            <div className="flex items-center justify-between mb-6">
              ...
              <Badge className="bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border-none font-bold text-[10px] uppercase">
                Ready to Import
              </Badge>
            </div>
              ...
              {normalizedFonts && normalizedFonts.length > 0 && (
                <div className="space-y-3">
                  <h4 className="text-xs font-bold text-muted-foreground uppercase tracking-wider flex items-center gap-2">
                    ...
                    <Badge className="bg-amber-500/10 text-amber-600 dark:text-amber-400 border-none font-bold text-[8px] uppercase">
                      Required
                    </Badge>
                  </h4>
                  <div className="space-y-2">
                    {normalizedFonts.map((font, idx) => (
                      ...
                      >
                        ...
                        <Badge
                          variant="outline"
                          className="text-[9px] border-primary/20 text-primary font-bold uppercase shrink-0"
                        >
                  ...
                  <div className="text-[11px] text-muted-foreground/80 space-y-1.5 p-3 bg-muted/30 rounded-lg">
                    <p className="font-semibold text-foreground/80">
                      How to install fonts in Webflow:
                    </p>
                    <ol className="list-decimal list-inside space-y-1 pl-1">
                      ...
                      <li>
                        Click <strong>Add fonts</strong> → Select{" "}
                        <strong>Google Fonts</strong> (or Adobe)
                      </li>
                      <li>Search for each font name above</li>
                      <li>Select the required weights shown above</li>
                      <li>
                        <strong>Publish</strong> your site to apply fonts
                      </li>
                    </ol>
              ...
              {securityWarnings.length > 0 && (
                <div className="space-y-3 p-5 bg-red-500/5 rounded-[24px] border border-red-500/20 ring-1 ring-red-500/10 shadow-inner">
                  ...
                  <p className="text-[11px] text-muted-foreground font-medium leading-tight">
                    {securityWarnings.length} external resource
                    {securityWarnings.length > 1 ? "s were" : " was"} removed
                      ...
                      >
                        <div className="flex items-start gap-2">
                          <Badge className="bg-red-500/10 text-red-400 border-none font-bold text-[8px] uppercase shrink-0 mt-0.5">
                            {warning.type === "blocked_cdn"
                              ? "BLOCKED"
                              : "CORS"}
                          </Badge>
                          <div className="flex-1 min-w-0">
                            <code className="text-muted-foreground font-mono break-all block">
                              {warning.url}
                            </code>
              ...
              {siteSafetyReport && (
                <div className="space-y-3 p-5 bg-white/5 rounded-[24px] border border-white/10 ring-1 ring-white/5 shadow-inner">
                  ...
                  <p className="text-[11px] text-muted-foreground font-medium leading-tight">
                    {siteSafetyReport.status === "pass" &&
                    securityWarnings.length === 0
                      ? "Payload is safe to paste."
                      : "Export contains minor warnings."}
                  </p>
                    ...
                    <Button
                      ...
                      onClick={async () => {
                        ...
                        if (siteSafetyReport.fatalIssues.length > 0) {
                          ...
                          lines.push(
                            ...siteSafetyReport.fatalIssues.map(
                              (issue) => `- ${issue}`,
                            ),
                          );
                        }
                        ...
                        if (siteSafetyReport.autoFixes.length > 0) {
                          ...
                          lines.push(
                            ...siteSafetyReport.autoFixes.map(
                              (fix) => `- ${fix}`,
                            ),
                          );
                        }
                        ...
                        if (
                          siteSafetyReport.embedSize.errors.length > 0 ||
                          siteSafetyReport.embedSize.warnings.length > 0
                        ) {
                          lines.push("Embed Size:");
                          lines.push(
                            ...siteSafetyReport.embedSize.errors.map(
                              (issue) => `- ${issue}`,
                            ),
                          );
                          lines.push(
                            ...siteSafetyReport.embedSize.warnings.map(
                              (issue) => `- ${issue}`,
                            ),
                          );
                        }
                        ...
                        if (
                          siteSafetyReport.extractedToEmbeds?.warnings?.length
                        ) {
                          lines.push("Embed Extraction:");
                          lines.push(
                            ...siteSafetyReport.extractedToEmbeds.warnings.map(
                              (warning) => `- ${warning}`,
                            ),
                          );
                        }
                        ...
                        if (
                          siteSafetyReport.htmlSanitization.warnings.length >
                            0 ||
                          siteSafetyReport.htmlSanitization.errors.length > 0
                        ) {
                          lines.push("HTML Sanitization:");
                          lines.push(
                            ...siteSafetyReport.htmlSanitization.errors.map(
                              (issue) => `- ${issue}`,
                            ),
                          );
                          lines.push(
                            ...siteSafetyReport.htmlSanitization.warnings.map(
                              (issue) => `- ${issue}`,
                            ),
                          );
                        }
        ...
        <div className="relative flex flex-col md:flex-row md:items-center justify-between gap-8">
          <div className="space-y-3">
            <div className="flex items-center gap-3">
              ...
              <Badge className="bg-primary/20 text-primary border border-primary/20 font-bold text-[10px] uppercase backdrop-blur-md">
                Base Layout + Styles
              </Badge>
            </div>
            <p className="text-muted-foreground font-medium max-w-xl text-lg leading-relaxed">
              Webflow natively understands this structure. Copy the full payload
              and paste it directly into your Webflow Canvas.
            </p>
          </div>
          ...
          <div className="flex items-center shrink-0">
            <Button
              ...
              className="bg-primary hover:opacity-90 text-primary-foreground shadow-lg shadow-primary/20 font-black px-8 sm:px-12 h-12 sm:h-14 rounded-xl sm:rounded-2xl text-base sm:text-lg hover:scale-[1.02] active:scale-[0.98] transition-all"
      ...
      <div className="space-y-6 pt-6 border-t border-border">
        <div className="flex items-center justify-between mb-2">
          ...
          <p className="text-sm font-medium text-muted-foreground/60">
            Extra resources for your project
          </p>
        </div>
        ...
        <Tabs defaultValue="images" className="w-full">
          ...
          <div className="animate-in fade-in slide-in-from-bottom-2 duration-300">
            <TabsContent value="images" className="mt-0">
              {hasImages ? (
                <ImagesGrid images={project.images} projectId={project._id} />
              ) : (
                ...
                <>
                    ...
                    <div className="bg-card/80 backdrop-blur-xl rounded-[32px] border border-white/20 ring-1 ring-white/10 p-8 shadow-2xl shadow-primary/5 space-y-6">
                      <div className="space-y-2">
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-3">
                            ...
                            <Badge className="bg-orange-500/10 text-orange-600 dark:text-orange-400 border-none font-bold text-[10px] uppercase">
                              {externalLibraries.length}{" "}
                              {externalLibraries.length === 1
                                ? "library"
                                : "libraries"}
                            </Badge>
                          </div>
                          ...
                          <Button
                            ...
                            onClick={async (e) => {
                              ...
                              const allTags = externalLibraries
                                .map((lib) =>
                                  lib.type === "script"
                              ...
                              toast.success(
                                `Copied ${externalLibraries.length} library tags`,
                              );
                            }}
                          >
                            Copy All Tags
                        ...
                        <p className="text-muted-foreground font-medium leading-relaxed">
                          Add these CDN links to your Webflow project.{" "}
                          <strong>Stylesheets</strong> go in{" "}
                          <code className="text-xs bg-muted px-1 py-0.5 rounded">
                            Site Settings → Custom Code → Head Code
                          </code>
                          . <strong>Scripts</strong> go in{" "}
                          <code className="text-xs bg-muted px-1 py-0.5 rounded">
                      ...
                      <div className="space-y-3">
                        {externalLibraries.map((lib, idx) => {
                          ...
                          return (
                            <div
                              key={idx}
                              className="flex items-center justify-between p-4 bg-white/5 rounded-xl border border-white/10 ring-1 ring-white/5 hover:bg-white/10 transition-all group"
                            >
                              <div className="flex items-center gap-3 flex-1 min-w-0">
                                <Badge
                                  className={
                                    lib.type === "script"
                                      ? "bg-yellow-500/10 text-yellow-500 border-none font-bold text-[10px] uppercase shrink-0"
                                      : "bg-purple-500/10 text-purple-400 border-none font-bold text-[10px] uppercase shrink-0"
                                  }
                                >
                  ...
                  <div className="bg-card/80 backdrop-blur-xl rounded-[32px] border border-white/20 ring-1 ring-white/10 p-8 shadow-2xl shadow-primary/5 space-y-8">
                    <div className="space-y-3">
                      ...
                      <p className="text-muted-foreground font-medium leading-relaxed">
                        Complex CSS and functional JavaScript that Webflow
                      ...
                      <div className="p-4 bg-blue-500/5 rounded-xl border border-blue-500/20 space-y-3">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
                          <div className="space-y-1.5">
                            <Badge className="bg-purple-500/10 text-purple-400 border-none font-bold text-[10px] uppercase">
                              CSS Embed
                            </Badge>
                            <ol className="list-decimal list-inside space-y-0.5 text-muted-foreground pl-1">
                              ...
                              <li>
                                Search for <strong>"Embed"</strong>
                              </li>
                              <li>
                                Drag to <strong>TOP of page</strong> (above all
                                content)
                              </li>
                          ...
                          <div className="space-y-1.5">
                            <Badge className="bg-yellow-500/10 text-yellow-500 border-none font-bold text-[10px] uppercase">
                              JS Scripts
                            </Badge>
                            <ol className="list-decimal list-inside space-y-0.5 text-muted-foreground pl-1">
                              <li>
                                Click <strong>Page Settings</strong> (gear icon)
                              </li>
                              <li>
                                Scroll to <strong>Custom Code</strong>
                              </li>
                              <li>
                                Paste in{" "}
                                <strong>Before &lt;/body&gt; tag</strong>
                              </li>
                      ...
                      {embeds.length > 0 ? (
                        embeds.map((embed, idx) => {
                          ...
                          return (
                            <div
                              key={idx}
                              className="p-8 bg-white/5 rounded-[28px] border border-white/10 ring-1 ring-white/5 space-y-6 transition-all hover:bg-white/10 hover:shadow-lg hover:shadow-primary/5"
                            >
                              <div className="flex items-center justify-between">
                                <div className="flex items-center gap-3">
                                  <Badge
                                    className={
                                      embed.type === "CSS"
                                        ? "bg-purple-500/10 text-purple-400 border-none font-bold uppercase text-[10px]"
                                        : "bg-yellow-500/10 text-yellow-500 border-none font-bold uppercase text-[10px]"
                                    }
                                  >
                                    {embed.type}
                                  </Badge>
                                ...
                                <div className="flex items-center gap-3">
                                  ...
                                  <Button
                                    ...
                                    className={
                                      copiedEmbed === idx
                                        ? "text-emerald-500 font-bold hover:bg-emerald-500/10 rounded-lg h-9 px-4"
                                        : "text-primary font-bold hover:bg-primary/5 rounded-lg h-9 px-4"
                                    }
                                    onClick={async (e) => {
                                      e.preventDefault();
