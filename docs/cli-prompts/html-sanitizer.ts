 ...
 */
export function prepareHTMLForWebflow(html: string): {
  sanitizedHTML: string;
  validation: ReturnType<typeof validateHTMLForWebflow>;
} {
  const sanitized = sanitizeHTMLForWebflow(html);
...
const rawHTML = '<span>Years<br>Experience</span>';
const safe = sanitizeHTMLForWebflow(rawHTML);
console.log(safe); // <span>Years</span><br><span>Experience</span>

// Example 2: Validation before conversion
const result = prepareHTMLForWebflow(rawHTML);
if (!result.validation.valid) {
  console.error('Validation failed:', result.validation.errors);
} else {
  // Send result.sanitizedHTML to Claude API
  convertToWebflow(result.sanitizedHTML);
}

// Example 3: In Convex mutation
import { mutation } from "./_generated/server";
import { sanitizeHTMLForWebflow } from "./html-sanitizer";

export const convertHTMLToWebflow = mutation({
  handler: async (ctx, args: { html: string }) => {
    // Step 1: Sanitize
    const sanitized = sanitizeHTMLForWebflow(args.html);
    
    // Step 2: Convert
    const payload = await callClaudeAPI(sanitized);
...
// Example 4: In frontend before upload
import { prepareHTMLForWebflow } from '@/lib/html-sanitizer';

const handleConvert = async () => {
  const result = prepareHTMLForWebflow(userHTML);
  
  if (!result.validation.valid) {
    alert(`HTML has issues:\n${result.validation.errors.join('\n')}`);
    return;
  }
  ...
  await convertToWebflow(result.sanitizedHTML);
};
*/
